-- After this migration, also run supabase-content-blocks-grid-nesting.sql
-- (grid = root, sub-blocks = heading | text_block with parent_id).
--
-- ============================================
-- Upgrade: content_blocks (new schema) + migration from text_blocks
...
-- New table (skipped if it already exists)
...
-- Migration: only if the old text_blocks table still exists
do $$
begin
