...
export function ContentStructureProvider({ children }: { children: ReactNode }) {
  ...
  const subBlockDialogTitle = useMemo(() => {
    if (editingBlock) return "Edit sub-block"
    if (subBlockTarget) {
      return `New sub-block · Cell ${subBlockTarget.cellIndex + 1}`
    }
    return "Sub-block"
  ...
  const setGridBackground = useCallback(
    async (grid: ContentBlock, enabled: boolean) => {
      ...
      try {
        ...
        toast.success(enabled ? "Grid background on" : "Grid background off")
        await loadData()
      } catch (e) {
        toast.error(e instanceof Error ? e.message : "Could not save grid")
      }
    },
    [loadData]
  )
  ...
  return (
    <ContentStructureContext.Provider value={value}>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Paste the full {'<svg>…</svg>'} element. Optional; it can be used on
                the site instead of or together with the cover.
              </p>
            </div>
            ...
            <DialogTitle>
              {editingSub
                ...
                : subDialogCategoryName
                  ? `New subcategory — ${subDialogCategoryName}`
                  : "New subcategory"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
      ...
      >
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>New grid</DialogTitle>
            <DialogDescription>
              Cell layout across the full section width. Add sub-blocks with
              the cell buttons after saving.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="space-y-2">
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <Label htmlFor="new-grid-has-background">Grid background</Label>
              <Switch
        ...
        >
          <SheetHeader className="p-0 pr-10">
            ...
            {editingBlock && editBlockPlacementLabel ? (
              <SheetDescription>
                Placement: {editBlockPlacementLabel}
              </SheetDescription>
            ) : null}
          </SheetHeader>
