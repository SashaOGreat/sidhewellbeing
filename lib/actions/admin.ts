...
async function persistManagedImageChange(
  ...
  if (!managed) throw new Error("Image record not found")
...
/** Upload a category/subcategory cover: Replicate remove-bg → PNG in storage (SVG is uploaded as-is). */
export async function uploadCategoryCoverImage(formData: FormData): Promise<string> {
  ...
  if (!process.env.REPLICATE_TOKEN) {
    throw new Error("REPLICATE_TOKEN is missing from the environment")
  }
...
/** Sets `display_order` from the ID order (0, 1, 2, …). */
...
/** Subcategory order within a category (`display_order` 0, 1, …). */
...
function normalizePriceListScope(formData: FormData) {
  ...
  if ((categoryId && subcategoryId) || (!categoryId && !subcategoryId)) {
    throw new Error("A price list row must belong to either a category or a subcategory")
  }
...
// ============================================
// CONTENT BLOCKS (page grid + sub-blocks in cells)
// ============================================
...
export async function createContentBlock(formData: FormData) {
  ...
  if (parent_id) {
    if (
      ...
      throw new Error("Invalid sub-block type")
    }
    ...
    if (!parentRow) throw new Error("Parent grid not found")
    ...
    if (block_type !== "grid") {
      throw new Error("Only a grid belongs on the page — use New grid.")
    }

    const layout = data.layout
    if (typeof layout !== "string") {
      throw new Error("Grid layout is missing")
    }
...
export async function deleteContentBlock(id: string) {
  ...
  if (row.parent_id === null && row.block_type === "grid") {
    ...
    if (count !== null && count > 0) {
      throw new Error(
        "A grid with content cannot be deleted. Remove all sub-blocks first."
      )
    }
  }
...
export async function createReusableSection(formData: FormData) {
  ...
  const slugBase = slugify(name) || `section-${Date.now()}`
...
export async function createReusableSectionNode(formData: FormData) {
  ...
  if (!section_id) throw new Error("Section is missing")
  ...
  if (parent_id) {
    ...
    if (!parentRow || parentRow.section_id !== section_id) {
      throw new Error("Parent grid not found")
    }
    ...
    if (block_type !== "grid") {
      throw new Error("Only a grid can be the root of a section")
    }
...
export async function deleteReusableSectionNode(id: string) {
  ...
  if (row.parent_id === null && row.block_type === "grid") {
    ...
    if (cErr) throw new Error(cErr.message)
    if (count !== null && count > 0) {
      throw new Error("A grid with content cannot be deleted.")
    }
  }
